(() => {
  ...
  async function startGroupCall() {
    ...
    try {
      ...
      // ИСПРАВЛЕНИЕ: Сначала показываем UI, потом создаем комнату
      isCallInitiator = true;
      showVideoCallUI();

      // Затем создаем комнату на сервере
  ...
  function handleCallStarted(message) {
    ...
    // ИСПРАВЛЕНИЕ: Убедимся, что UI видеозвонка показан
  ...
  function handleRoomCreated(message) {
    ...
    // ИСПРАВЛЕНИЕ: Всегда показываем UI видеозвонка при создании комнаты
    showVideoCallUI();
  ...
  async function createOffer(targetSessionId, attempt = 1) {
    ...
    if (peerConnections.has(targetSessionId)) {
      ...
      // Если соединение уже установлено или устанавливается, не создаем новый offer
      if (
  ...
  async function handleRoomUsers(message) {
    ...
    // ИСПРАВЛЕНИЕ: Упрощенная логика - создаем соединения со всеми пользователями
    for (let i = 0; i < otherUsers.length; i++) {
      ...
      if (!peerConnections.has(user.sessionId)) {
        ...
        try {
          // ВСЕГДА создаем offer для новых пользователей
  ...
  async function startIndividualCall(targetUserId) {
    if (isInCall) {
      showSystemMessage("❌ Вы уже в звонке");
      return;
    }
    ...
    try {
      ...
      // ИСПРАВЛЕНИЕ: Сначала инициализируем медиапоток и показываем UI
      await initializeLocalStream();
      showVideoCallUI();
      ...
      showSystemMessage("📞 Вызываем пользователя...");
      console.log("✅ Individual call initialization started");
    } catch (error) {
  ...
  function updateParticipantsCount(count) {
    ...
    // ИСПРАВЛЕНИЕ: Всегда показываем минимум 1 участника (себя)
    ...
    console.log(
      "🔍 Room users:",
      Array.from(roomUsers.entries()).map(([id, user]) => user.userName)
    );
  ...
  function showVideoCallUI() {
    ...
    // Показываем контейнер видеозвонка
    videoCallContainer.classList.remove("hidden");

    // ИСПРАВЛЕНИЕ: Принудительно обновляем отображение
    ...
    // ИСПРАВЛЕНИЕ: Принудительно обновляем счетчик участников
  ...
  function hideVideoCallUI() {
    ...
    // Скрываем контейнер видеозвонка
    videoCallContainer.classList.add("hidden");

    console.log("✅ Video call UI hidden");
  }
