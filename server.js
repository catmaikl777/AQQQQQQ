// server.js (исправленная версия с правильной проверкой имен)
...
const db = {
  ...
  async saveFileMessage(
    ...
    const client = await pool.connect();
    try {
      // Проверяем и создаем колонки если нужно
      await this.ensureFileColumns(client);
  ...
  // Новая функция для проверки и создания колонок
  ...
  async getMessageHistory(limit = 100) {
    const client = await pool.connect();
    try {
      // Сначала проверяем существование файловых колонок
    ...
    `);
      ...
      let fileColumnsSelect = "";
      if (hasFileColumns) {
        fileColumnsSelect = ", m.file_name, m.file_type, m.file_size";
      }
  ...
  // ИСПРАВЛЕННАЯ ФУНКЦИЯ - проверяем занято ли имя ДРУГИМ пользователем
  async isUsernameAvailable(userId, newUsername) {
    ...
    try {
      ...
      return result.rows.length === 0; // true если имя свободно
    } catch (error) {
...
const server = http.createServer((req, res) => {
  ...
  console.log(`Request for: ${filePath}, Full path: ${fullPath}`); // Логирование для отладки
...
wss.on("connection", async (ws, req) => {
  ...
  try {
    // Создаем временного пользователя с уникальным именем
      ...
      switch (message.type) {
        case "setName":
          if (message.name && message.name.trim()) {
            ...
            try {
              // ИСПРАВЛЕННАЯ ЛОГИКА: проверяем, не занято ли имя ДРУГИМ пользователем
