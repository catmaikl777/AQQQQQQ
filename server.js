// server.js - исправленная версия с правильным управлением сессиями
...
const db = {
  ...
  // ИСПРАВЛЕНИЕ: Правильная очистка дублирующихся сессий
  async cleanupDuplicateSessions(userId, currentSessionId) {
    const client = await pool.connect();
    try {
      // Закрываем все активные сессии для этого пользователя, кроме текущей
      await client.query(
      ...
      // Удаляем из памяти все соединения для этого пользователя, кроме текущего
      clients.forEach((clientData, sessionId) => {
        if (clientData.userId === userId && sessionId !== currentSessionId) {
          try {
            if (clientData.ws.readyState === WebSocket.OPEN) {
              clientData.ws.close(1000, "Duplicate session closed");
            }
          } catch (error) {
      ...
      console.log(`🧹 Cleaned up duplicate sessions for user ${userId}, keeping only ${currentSessionId}`);
    } catch (error) {
  ...
  async createUserSession(userId, sessionId) {
    const client = await pool.connect();
    try {
      // Сначала закрываем любые существующие сессии для этого пользователя
  ...
  // ИСПРАВЛЕНИЕ: Правильное получение онлайн пользователей
...
wss.on("connection", async (ws, req) => {
  ...
  try {
    ...
    // ИСПРАВЛЕНИЕ: Сначала очищаем дублирующиеся сессии
