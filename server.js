...
const db = {
  async init() {
    ...
    try {
      ...
      // Создаем таблицу сообщений с поддержкой файлов (исправленная версия)
      await client.query(`
  ...
  // ИСПРАВЛЕННАЯ ФУНКЦИЯ - добавлен content для файлов
  async saveFileMessage(userId, filename, filetype, size, data, targetUserId = null) {
    ...
    try {
      ...
      // Используем filename как content для файловых сообщений
      const result = await client.query(
      ...
      switch (message.type) {
        ...
        case "call_busy":
          if (message.roomId && message.fromUserId) {
            const fromClient = Array.from(clients.values()).find(client => client.userId === message.fromUserId);
        ...
        case "file":
          if (message.filename && message.data) {
            try {
              ...
              // ИСПРАВЛЕННЫЙ ВЫЗОВ - передаем все необходимые параметры
              ...
              ws.send(JSON.stringify({ type: "system", text: "Ошибка при отправке файла" }));
            }
          }
          break;
        ...
        case "action":
          if (message.text && message.text.trim()) {
            ...
            broadcast({ type: "action", name: currentUser.username, text: text });
          }
          break;
        ...
        case "reaction":
          if (message.emoji) {
            ...
            broadcast({ type: "reaction", name: currentUser.username, emoji: message.emoji });
          }
          break;
          ...
          if (message.to && message.text && message.text.trim()) {
            ...
            if (targetUser) {
              ...
              if (targetClient && targetClient.ws.readyState === WebSocket.OPEN) {
                ...
                ws.send(JSON.stringify({ type: "system", text: "Пользователь не в сети" }));
              }
            }
          }
          break;
        ...
        case "start_individual_call":
          if (message.targetUserId) {
            const targetClient = Array.from(clients.values()).find(client => client.userId === message.targetUserId);
            if (targetClient && targetClient.ws.readyState === WebSocket.OPEN) {
              const roomId = `room_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
              rooms.set(roomId, {
                users: new Map([
