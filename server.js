...
wss.on("connection", async (ws, req) => {
  ...
  try {
    ...
    ws.send(
      JSON.stringify({ type: "init", id: userId, name: currentUser.username })
    );
    ...
    try {
      ...
      switch (message.type) {
        ...
        case "create_room":
          ...
          // Уведомляем всех пользователей о создании комнаты (групповой звонок)
          broadcast(
            {
              type: "call_invite",
              fromUserId: userId,
        ...
        // Добавьте этот case для индивидуальных звонков
      ...
      ws.send(
        JSON.stringify({ type: "system", text: "Ошибка обработки сообщения" })
      );
    }
  });
  ...
  ws.on("close", async () => {
    ...
    if (clientData) {
      ...
      broadcast({
        type: "system",
        text: `${currentUser.username} вышел из чат`,
      });
      await broadcastUsers();
    }
  });
